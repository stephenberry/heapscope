...
- ~~**`SymFromAddr` has never been executed anywhere.**~~ **Closed by the first native Windows run.** It cannot run under Wine — the first call kills the test process with `rosetta error: invalid gdt selector index 5`, from Apple's translator rather than from Wine, which says nothing about whether the code is right — so this stood open for as long as Wine was the only Windows available. `windows-latest` ran it: all seven `symbol::dynamic` tests pass, including the sentinel-address case and the module-map gate. `HEAPSCOPE_SYMBOLIZE=0` remains, justified independently: dbghelp honours `_NT_SYMBOL_PATH`, so a profile written at exit can block on a network symbol server; two machines produce different frame text for one program, which makes profiles awkward to diff; and an emulator may be unable to run it at all.
...
- ~~**Offline symbolization has never worked on Windows.**~~ **Closed, and the divergence it rests on is permanent.** `Module::bias` there is the image base, so a recorded file address is a relative virtual address rather than the address the file records: `llvm-symbolizer` needs `--relative-address`, and `addr2line`, which resolves against section VMAs, cannot be used at all. Both `tests/end_to_end.rs` and `heapscope-symbolize` now know that — the tool passes the flag and drops `addr2line` from its Windows preference list — and the native run resolves the function that allocated. What does **not** close is the divergence itself: `bias` still means a different quantity on Windows than on ELF and Mach-O, for the reason immediately below.

  **And the obvious fix does not work, which is the durable finding.** Reading the image's own PE header at its base — what the Mach-O and ELF backends do, and what makes their `bias` the link-time slide — returns the *load* address on Windows, because the loader rewrites `ImageBase` in the mapped optional header when it relocates. **Measured**: ASLR placed one binary at two different bases across two CI runs and the field equalled the load address both times, giving a `bias` of zero and a file address identical to the runtime one. Tried, measured, reverted. Closing it needs the base out of the file on disk — the same tier-3 requirement as the entry below, reached by an unrelated route.
- **The macOS shared-cache bias is wrong for `llvm-symbolizer` and `addr2line`**, and fixing it needs tier 3.
- ~~**CI has never completed a run.**~~ **Closed.** It could not start at all while the repository was private — every job failed with an account-payment annotation, which is why the four-platform matrix had only ever been verified locally. Making the repository public cleared it, and the matrix now runs on every push: macOS aarch64, Linux x86_64, Linux aarch64, and Windows natively, plus Miri, both sanitizers, the viewer job and the invariants job.

  Getting there took eleven fixes, and every one was a defect CI found rather than a defect CI introduced — the kind this document's own record predicts. Line endings (`.gitattributes`, absent, so a CRLF checkout changed an `include_str!`-embedded asset and silently disarmed a durability test); two `-D warnings` lints that only fire under nightly or `cfg(miri)`; two guards whose predicates measured something adjacent to what they asserted; a viewer check whose clicked row could not falsify the bug it existed for; `nm` and `/dev/null` assumed to behave as they do on unix; an exit-status rule that fired on Windows system images with no PDB; and four separate ThreadSanitizer problems, of which the first — 131 locks against its 128-slot deadlock detector — had made that job appear to hang for two hours on every run since the beginning.
...
- `[dependencies]` empty, builds on MSRV 1.96 with the library-only check, clean on the four-platform matrix. **Audited**: the command all four `Test (…)` jobs run could not exit zero on any platform, and CI had never completed a run to notice. **Met**: the matrix now passes on every push, Windows natively rather than under Wine. Eleven fixes stood between the audit and that — see the CI entry in §9.1, which lists them, because the shape they share is this document's oldest lesson arriving all at once.
