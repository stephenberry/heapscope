...
- **Offline symbolization has never worked on Windows**, which is the other half of the Windows gap and was lost when this section was condensed. `Module::bias` there is the image base, so the difference is a relative virtual address rather than the address the file records: `llvm-symbolizer` needs `--relative-address` and `addr2line`, which resolves against section VMAs, cannot be used at all. `heapscope-symbolize` passes neither and so does not work there either.
