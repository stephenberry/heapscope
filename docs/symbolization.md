...
**In-process symbolization on Windows is verified by a native run.** It could not be for a long time: `SymFromAddr` cannot execute under Wine — see `ci/windows-under-wine.sh` — so it was compiled and reviewed against Microsoft's documentation and nothing more. CI now runs the suite on `windows-latest`, where all seven `symbol::dynamic` tests pass, including the module-map gate and the sentinel address `dladdr` answers wrongly on other platforms.

Offline symbolization works there too, with one difference worth knowing: `Module::bias` on Windows is the image base, so a recorded file address is a *relative* virtual address. `llvm-symbolizer` takes one with `--relative-address`, which `heapscope-symbolize` passes for you; `addr2line` resolves against section VMAs and cannot be used on Windows at all.
